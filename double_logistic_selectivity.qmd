...
extension of monotone logistic selectivity because they can represent declining
vulnerability at older ages while remaining low-dimensional [@punt2013].
Interpretation still requires care: dome shape can be confounded with natural
mortality and cohort processes if independent information is weak [@thompson1994].
...
The scenario summary in [Table](#tbl-dl-scenarios) lists the four parameter sets and the
derived inflection points ($\gamma_1$, $\gamma_2$) used in the selectivity
scenarios.

## Faceted scenario curves

Each scenario appears on its own panel in [Figure](#fig-dl-faceted) so the ascent,
...
All scenarios are overlaid in [Figure](#fig-dl-overlay) to highlight differences in peak
...
The three panels in [Figure](#fig-dl-sensitivity) show how each parameter changes the
...
The MCMC chunks in this notebook are currently set with `eval: false`, so these
figures and posterior summaries are not rendered by default. To generate them,
set `eval: true` for the MCMC chunks and render the document.
...
The marginal posterior densities in [Figure](#fig-dl-marginals) summarize $(p_1, p_2,
...
This section is also code-defined with `eval: false`; therefore, no additional
prior-set outputs appear in default renders unless those chunks are enabled.
